...
### Penjelasan
Materi tentang **Variabel Dan Konstanta** dalam bahasa pemrograman Go. Konsep ini adalah salah satu fondasi penting saat Anda mulai mengembangkan aplikasi dari tahap *beginner* ke level *production-grade*.

Go didesain untuk kesederhanaan dan kejelasan, dan fitur terkait `Variabel Dan Konstanta` direkayasa sedemikian rupa agar sangat performan dengan *overhead* memori dan eksekusi serendah mungkin dibandingkan dengan implementasi di bahasa *scripting* konvensional.
...
func main() {
    fmt.Println("Ini adalah demonstrasi materi: Variabel Dan Konstanta")
    // TODO: Implementasi logika Variabel Dan Konstanta di sini
}
```

### Praktik
Buatlah sebuah *package* mandiri (standalone package) Go, eksplorasi bagaimana Variabel Dan Konstanta berjalan. Buat sebuah modul fungsional yang menyertakan penanganan *error* yang baik.
